...
from app.models.entities import TaskRun, TaskOutput, MonitorTask
...
async def get_sov_trend(
    ...
    query = select(
        ...
        func.avg(TaskOutput.sov_score).label('avg_sov'),
        func.count(TaskOutput.id).label('count')
    ).select_from(
        TaskRun.__table__.join(TaskOutput.__table__)
    ...
    # 添加过滤条件
    if keyword:
        query = query.where(TaskOutput.keyword == keyword)
    
    if model:
        query = query.where(TaskOutput.model_name == model)
...
async def get_accuracy_trend(
    ...
    query = select(
        func.date(TaskRun.completed_at).label('date'),
        func.avg(TaskOutput.accuracy_score).label('avg_accuracy'),
        func.count(TaskOutput.id).label('count')
    ).select_from(
        TaskRun.__table__.join(TaskOutput.__table__)
        .join(MonitorTask.__table__)
...
async def get_metrics_summary(
    ...
    # 平均SOV分数
    avg_sov_query = select(func.avg(TaskOutput.sov_score)).select_from(
        TaskOutput.__table__.join(TaskRun.__table__)
        .join(MonitorTask.__table__)
    ).where(
        and_(
            MonitorTask.tenant_id == tenant_id,
    ...
    # 平均准确性分数
    avg_accuracy_query = select(func.avg(TaskOutput.accuracy_score)).select_from(
        TaskOutput.__table__.join(TaskRun.__table__)
        .join(MonitorTask.__table__)
    ).where(
        and_(
...
async def get_model_comparison(
    ...
    ).group_by(TaskOutput.model_name)
...
async def get_keyword_performance(
    ...
    ).group_by(TaskOutput.keyword).order_by(
        desc(func.avg(TaskOutput.sov_score))
    ).limit(limit)
