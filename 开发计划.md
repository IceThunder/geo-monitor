...
- [x] **数据迁移脚本**
  - 编写数据库初始化脚本 (database/init.sql 已合并完善)
  - Alembic 迁移工具已初始化 (backend/alembic/)
  - 创建测试数据生成器 (通过 pytest conftest.py fixtures)
  - 实现数据备份和恢复机制
...
- [x] **任务调度优化**
  - APScheduler + Redis 队列任务调度 (backend/app/worker.py)
  - 任务失败重试机制 (executor.py 中重试逻辑)
  - 任务执行监控 (WebSocket 实时推送 + NotificationCenter)
...
- [x] **CI/CD流水线**
  ...
  - 自动化测试 (pytest backend, npm lint+build frontend)
  - 自动化部署 (待配置)
  - 监控和告警 (Grafana 已在 docker-compose 中配置)
...
POST   /api/tasks/{task_id}/run      # 手动执行任务

# 指标分析
GET    /api/metrics/dashboard        # 仪表盘数据
GET    /api/metrics/sov              # SOV趋势数据
GET    /api/metrics/accuracy         # 准确性数据
GET    /api/metrics/comparison       # 模型对比数据

# 告警管理
GET    /api/alerts                   # 获取告警列表
POST   /api/alerts/rules             # 创建告警规则
PUT    /api/alerts/rules/{rule_id}   # 更新告警规则
POST   /api/alerts/test              # 测试告警配置

# 配置管理
GET    /api/config/models            # 获取可用模型列表
POST   /api/config/credentials       # 配置API密钥
GET    /api/config/usage             # 获取使用统计
```
...
SELECT cron.schedule('geo-monitor-scheduler', '*/5 * * * *', 
    'SELECT net.http_post(
        url := ''https://your-backend.railway.app/api/scheduler/trigger'',
...
async def test_task_execution_flow():
    ...
    # 2. 手动触发执行
    response = await client.post(f"/api/tasks/{task.id}/run")
    ...
    # 4. 验证结果
    metrics = await client.get(f"/api/metrics/task/{task.id}")
